...
public interface RecipeMapper extends BaseMapper<Recipe> {
    ...
    @Select("""
            <script>
            SELECT r.product_id AS productId, i.name AS name
            FROM ingredient i
            JOIN recipe_ingredient ri ON ri.ingredient_id = i.id
            JOIN recipe r ON r.id = ri.recipe_id
            WHERE r.product_id IN
            <foreach collection='productIds' item='id' open='(' separator=',' close=')'>#{id}</foreach>
            ORDER BY r.product_id, ri.display_order
            </script>
            """)
    List<Map<String, Object>> selectIngredientNamesByProductIds(@Param("productIds") Collection<Integer> productIds);
}
