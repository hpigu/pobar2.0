...
// WebSocket 即時推播，斷線時 polling 補位
const { connect } = useWebSocket('/topic/kitchen', () => {
  fetchItems()
  playNotify()
})
...
function statusLabel(s) {
  return { PENDING: '待製作', IN_PROGRESS: '製作中', READY: '完成' }[s] || s
}
...
<template>
  <div class="display-page">
    ...
    <div v-if="items.length === 0" class="empty-state">
      <el-empty description="目前沒有待處理的品項" />
    </div>
      ...
      <el-card v-for="item in items" :key="item.id"
        ...
        <div class="card-actions">
          <el-button v-if="item.status === 'PENDING'" type="warning" size="small"
            @click="updateStatus(item.id, 'IN_PROGRESS')">開始製作</el-button>
...
<style scoped>
.display-page { min-height: 100vh; background: #111; color: #fff; padding: 16px; }
.display-header {
...
.order-card { background: #1e1e2e !important; border: none !important; }
...
.table-label { font-weight: 700; font-size: 16px; color: #fff; }
.item-name { font-size: 20px; font-weight: 700; color: #fff; margin: 4px 0; }
.item-qty { font-size: 32px; font-weight: 900; color: #e6a23c; }
