...
```bash
cp .env.example .env
# 編輯 .env，填入 MYSQL_ROOT_PASSWORD、DB_PASSWORD、JWT_SECRET
# 若要自動建立 admin 帳號，填入 INIT_ADMIN_PASSWORD
```
...
3. 確認系統運作後，清空 `.env` 中的 `INIT_ADMIN_PASSWORD=` 並重啟：
...
> 電腦開機後 Docker Desktop 啟動完成（約 30 秒），即可雙擊「啟動系統」。
...
│   ├── 關閉系統.bat
│   └── 檢查狀態.bat
├── Dockerfile           # 後端 multi-stage build
├── docker-compose.yml
├── .env.example
└── README.md
```
